...
def main_list(params): # Main menu
    plugintools.log("freshstart.main_list "+repr(params)); yes_pressed=plugintools.message_yes_no(AddonTitle,"Do you wish to remove your Setup and restore settings?")
    if yes_pressed:
            ...
            for root, dirs, files in os.walk(xbmcPath,topdown=False):
                ...
                for name in dirs:
                    try: os.rmdir(os.path.join(root,name))
                        ...
                        plugintools.log("Error removing "+root+" "+name)
            if not failed: plugintools.log("freshstart.main_list All user files removed, you now have a clean install"); plugintools.message(AddonTitle,"ODIN Uninstall is complete, you're now back to a fresh Kodi configuration!","Please reboot your system or restart Kodi in order for the changes to be applied.")
    ...
    else: plugintools.message(AddonTitle,"Your setup","has not been changed"); plugintools.add_item(action="",title="Done",folder=False)
run()